        ...
        <div class="custom-page-header">
            <div class="header-left">
                ...
                <div class="header-content">
                    <h2 class="header-title">Manage Designations</h2>
                    <ol class="custom-breadcrumb">
                        ...
                        <li><i class="fa fa-angle-right"></i></li>
                        <li class="active">Designation Management</li>
                    </ol>
                </div>
            </div>
                ...
                <div class="col-12">
                    <div class="box new_table_box">
                        <div class="box-header">
                            <h4 class="box-title">Manage Designations</h4>
                            <div class="float-right" style="float:right;">
                                <button type="button" class="btn btn-primary-light btn-sm" id="open-designation-modal">Add +</button>
                            </div>
                        </div>
                        <div class="box-body">
...
<div class="modal modal-lg new_modal_design" id="designation-crud-modal" tabindex="-1" aria-labelledby="designation-crud-modalLabel" aria-hidden="true">
                    ...
                    <div class="header-content">
                        ...
                        <ol class="custom-breadcrumb">
                            <li>
                                ...
                                Fill in the details to add or update a designation.
                            </li>
                        </ol>
                    </div>
                </div>
...
<script>
    var data_table = $('#server-side-data-table').DataTable({
        ...
        ajax: {
            url: '<?php echo base_url('get-designations-table') ?>',
            type: 'POST',
            data: function(d) {
    ...
    $('#open-designation-modal').click(function(e) {
        e.preventDefault();
        resetDesignationForm();
        $('#crud-modal-title').text('Add Designation');
        $('#action-btn').text('Create').attr('data-key', '');
        $('#designation-crud-modal').modal('show');
    });
    ...
    $(document).on('click', '#action-btn', function(e) {
        ...
        $.ajax({
            url: '<?php echo base_url();?>' + ((key === '') ? 'designation-add' : 'designation-update'),
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function(response) {
                ...
                if (response.status) {
                    ...
                    $('#designation-crud-modal').modal('hide');
                    data_table.draw();
                } else {
                    toastr.error(response.message || 'Failed to save designation');
                }
            },
            error: function() {
    ...
    $(document).on('click', '.edit-designation', function() {
        ...
        $.ajax({
            url: '<?php echo base_url('designation-details') ?>',
            type: 'POST',
            dataType: 'JSON',
            data: {
            ...
            success: function(response) {
                ...
                if (!response.status) {
                    toastr.error(response.message || 'Failed to fetch designation details');
                    return;
                }

                $('#crud-modal-title').text('Edit Designation');
