    ...
    <div class="container-full">
        ...
        <div class="custom-page-header">
            ...
            <div class="header-banner">
                <img src="<?php echo base_url('assets/new_img/lead_img.png'); ?>" alt="">
            </div>
        </div>
        <!-- Main content -->
                            ...
                            <div class="container mt-1">
                                ...
                                <form id="leadForm" novalidate>
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-12" id="restaurantReservationPlacement">
...
<script>
    ...
    function resetDynamicFields() {
        ...
        $('#dynamicFields').empty();
        $('#restaurantReservationPlacement .row').empty();
...
<div class="col-md-4">
    ...
    <input type="text" name="special_occasion" class="form-control">
            ...
            `);
                ...
                $('#restaurantReservationPlacement .row').html(`
    ...
    <div class="col-md-4 mt-3 mb-4">
        ...
        <input type="date" name="second_followup_date" class="form-control">
            ...
            if (department === 'restaurant') {
                container
                    .find('[name="followup_date"], [name="second_followup_date"]')
                    .closest('.col-md-4')
                    .removeClass('mt-3');
            }
...
`;
    ...
    function leadField(field) {
        const restaurantReservationFields = [
            'restaurant_reservation', 'booking_date', 'restaurant_id', 'table_category_id',
            'table_id', 'slot_type_id', 'time_slot_id', 'table_reservation_status'
        ];
        if (restaurantReservationFields.includes(field) && $('#openReserveTableModal').length) {
    ...
    function showLeadFieldError(field, message) {
        ...
        const isRestaurantReservationError = input.is('#openReserveTableModal');
        let error = isRestaurantReservationError
            ? $('#restaurant_reservation_error')
            : $('#' + field + '_error');
    ...
    function validateLeadForm() {
        ...
        if (disposition === 'Quotation Sent') {
            ...
            if (department === 'restaurant') {
                const hasReservation = value('booking_date')
                    && value('restaurant_id')
                    && value('slot_type_id')
                    && value('time_slot_id')
                    && value('table_category_id')
                    && $('#table_id').val()
                    && $('#table_id').val().length > 0
                    && value('table_reservation_status');
                if (!hasReservation) {
                    errors.restaurant_reservation = 'Please complete the table reservation.';
                }
            }
        }
                ...
                success: function(response) {
                    ...
                    if (response.duplicate) {
                        Swal.fire({
                            ...
                            window.location.href = '<?php echo base_url("manage-leads") ?>';
                        });
                        return;
                    }
...
<script>
    ...
    $(document).on("change", "#checkin_date", function() {
        ...
        if (checkin < today) {
            ...
            hideError($(this));
            $("#checkout_date").attr("min", checkin || today);
        }
    });
