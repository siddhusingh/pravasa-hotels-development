                     ...
                     <div class="row mt-2">
                        ...
                        <div class="col-md-3">
                           <select name="assigned_to" id="assigned_to_filter" class="form-control"
                              <?= $scoped_staff_id > 0 ? 'disabled aria-readonly="true"' : ''; ?>>
                              <option value="">All Assigned Users</option>

                              <?php foreach ($assigned_users as $user):
                                 $value = $user->id . '|' . $user->role;
                                 $selected = $scoped_staff_id > 0
                                    ? (((int) $user->id === $scoped_staff_id && $user->role === 'agent') ? 'selected' : '')
                                    : ((isset($_GET['assigned_to']) && $_GET['assigned_to'] == $value) ? 'selected' : '');
                              ?>
                  ...
                  <div class="box-body">
                     ...
                     <div class="">
                        ...
                        if (
                           ...
                           && empty($showfollowupleads)
                           && empty($scoped_staff_id)
                           && empty($this->input->get('phone'))
                        ) {
                           $current_status = 'Open';
                        }
                        ?>
...
<script>
   $(document).ready(function() {
      ...
      $(document).on("click", ".editLeadDetails", function() {
         ...
         csrfAjax({
            url: "<?= base_url('hotelAdmin/Leads/get_lead_details') ?>",
            type: "POST",
            data: {
               lead_id: lead_id
            },
            ...
            error: function(xhr) {
               ...
               toastr.error(response.message || 'Unable to load this lead.');
            },
            complete: function() {
               $("#processingLoader").hide(); // Hide loader ALWAYS
            }
         ...
         if (disposition === "Quotation Sent" || disposition === "Contacted") {
            ...
            if (disposition === "Quotation Sent" && department === "banquets") {
               ...
               container.append(`
                  ...
                  <div class="col-lg-3 col-md-6 col-sm-12 edit-room-required-date-fields" style="${roomDateDisplay}">
            ...
            success: function(response) {
               ...
               if (response.status) {
                  ...
                  Swal.fire({
                     icon: 'error',
                     title: 'Error',
                     text: response.message || 'Unable to update lead.',
...
<script>
   $(document).ready(function() {
      ...
      if (departmentFromGet.length > 0) {
         $("#department").val(departmentFromGet).trigger('change');
      }
      ...
      function fetchLeads(reset = false) {
         ...
         let filters = {
            ...
            assigned_id: scopedStaffId || assignedUser.id,
            assigned_role: scopedStaffId ? 'agent' : assignedUser.role,
         ...
         csrfAjax({
            ...
            success: function(response) {
               ...
               if (reset) {
                  $('#lead_container').html(
                     response.html || '<div class="text-center text-muted py-5">No leads found for the selected status.</div>'
                  );
               } else {
...
${data.booking_date && data.booking_date !== '0000-00-00' ? `
<div class="col-md-4 mb-3">
   ...
   <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg">
      <div class="modal-content">
         ...
         <div class="modal-body">
            ...
            <!-- Table -->
            <table class="table table-bordered" id="leadReportRoomsRateTable">
               <thead>
                  <tr>
                     <th>Date</th>
                     <th>Room Type</th>
...
<div id="leadReportProcessingLoader"
...
<script>
   ...
   function renderRateTable(data) {
      ...
      $("#leadReportRoomsRateTable tbody").html(html);
   }
</script>
