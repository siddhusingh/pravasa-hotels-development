...
class LeadController extends CI_Controller
{
    ...
    private function validateLeadInput($departmentName)
    {
        ...
        if ($disposition === 'Quotation Sent') {
            ...
            if ($department === 'restaurant') {
                ...
                $categoryId = $value('table_category_id');
                $slotTypeId = $value('slot_type_id');
                ...
                if (empty($selectionErrors) && $timeSlotId !== '') {
        ...
        if ($this->input->method() === 'post') {
            ...
            if (
                ...
                if ($department === 'restaurants') {
                    ...
                    // Keep the first table in the legacy column for existing reports.
                    // Every selected table is stored in lead_reserved_tables below.
                    $normalizedTableIds = $this->RestaurantBookingModel->normalizeTableIds($tableIds);
            ...
            // Insert the lead, then store each selected restaurant table separately.
            $insert_id = $this->LeadModel->insert_lead($leadData);

            if ($insert_id && $isRestaurantLead) {
                if (!$this->RestaurantBookingModel->replaceLeadTables($insert_id, $normalizedTableIds)) {
                    $insert_id = false;
                }
            }

            if ($restaurantTransactionStarted) {
                if ($insert_id && $this->db->trans_status()) {
    ...
    */
    ...
    public function get_lead_details()
    {
        ...
        if (empty($lead->table_ids) && !empty($lead->table_id)) {
            $lead->table_ids = [(int) $lead->table_id];
        }
    ...
    */
    ...
    public function update_lead()
    {
        ...
        $bookingDate = $dateValue('booking_date') ?: $dateValue('booking_enquiry_date');
        $followupDate = $dateValue('followup_date');
        $secondFollowupDate = $dateValue('second_followup_date');
        $validBookingDate = $bookingDate !== '' && $isValidDate($bookingDate);
        $validFollowupDate = $followupDate !== '' && $isValidDate($followupDate);
        $validSecondFollowupDate = $secondFollowupDate !== '' && $isValidDate($secondFollowupDate);
        $dateErrors = [];
        ...
        if ($followupDate !== '' && !$validFollowupDate) {
            $dateErrors['followup_date'] = 'Please enter a valid Follow-up Date.';
        }
        if ($secondFollowupDate !== '' && !$validSecondFollowupDate) {
            $dateErrors['second_followup_date'] = 'Please enter a valid 2nd Follow-up Date.';
        }
        if (($followupDate !== '' || $secondFollowupDate !== '') && $bookingDate !== '' && !$validBookingDate) {
            $dateErrors['booking_date'] = 'Please enter a valid Booking Date.';
        }
        if ($validBookingDate && $validFollowupDate && $followupDate >= $bookingDate) {
            $dateErrors['followup_date'] = 'Follow-up Date must be before Booking Date.';
        }
        if ($validBookingDate && $validSecondFollowupDate && $secondFollowupDate >= $bookingDate) {
            $dateErrors['second_followup_date'] = '2nd Follow-up Date must be before Booking Date.';
        }
        if ($secondFollowupDate !== '' && $followupDate === '') {
            $dateErrors['followup_date'] = 'Follow-up Date is required before entering a 2nd Follow-up Date.';
        } elseif ($validFollowupDate && $validSecondFollowupDate && $secondFollowupDate <= $followupDate) {
            $dateErrors['second_followup_date'] = '2nd Follow-up Date must be later than Follow-up Date.';
        }
        ...
        if (!empty($dateErrors)) {
            return $this->output
                ...
                ->set_output(json_encode([
                    'status' => false,
                    'message' => 'Please correct the highlighted follow-up dates.',
                    'error_message' => 'Please correct the highlighted follow-up dates.',
                    'errors' => $dateErrors,
                    'csrfHash' => $this->security->get_csrf_hash()
                ], JSON_UNESCAPED_UNICODE));
        }
        ...
        $department  = $department_data->department_name;
        $isRestaurantUpdate = in_array(
            strtolower(trim((string) $department)),
        ...
        $restaurantTransactionStarted = false;
        $hasRestaurantBookingFields = $isRestaurantUpdate
            && $this->input->post('restaurant_id') !== null
            && $this->input->post('time_slot_id') !== null;

        if ($hasRestaurantBookingFields) {
            $tableIds = $this->input->post('table_id');
            $normalizedTableIds = $this->RestaurantBookingModel->normalizeTableIds($tableIds);
            $selectionErrors = $this->RestaurantBookingModel->validateSelection(
            ...
            if (!empty($selectionErrors)) {
                return $this->output
                    ...
                    ->set_output(json_encode([
                        'status' => false,
                        'error_message' => 'Please correct the highlighted restaurant booking fields.',
        ...
        if ($updated && $hasRestaurantBookingFields) {
            $updated = $this->RestaurantBookingModel->replaceLeadTables($id, $normalizedTableIds);
        }
    ...
    */
    ...
    {
        $isSuperAdmin = !empty($this->session->userdata('super_admin_session'))
            && $this->session->userdata('role_as') === 'super_admin'
            && (int) $this->session->userdata('user_role') === 1;
        ...
        if (!$isSuperAdmin) {
            return $this->output
                ...
                ->set_output(json_encode([
                    'status' => false,
                    'message' => 'You are not authorized to permanently delete leads.',
                    'csrfHash' => $this->security->get_csrf_hash()
                ], JSON_UNESCAPED_UNICODE));
        }
        ...
        return $this->output
            ...
            ->set_output(json_encode([
                ...
                'message' => $deleted
                    ...
                    : 'Lead was not found or could not be permanently deleted.',
                'csrfHash' => $this->security->get_csrf_hash()
            ], JSON_UNESCAPED_UNICODE));
    }
    ...
    public function checkRestaurantAvailability()
    {
        ...
        $bookingDate = trim((string) $this->input->post('booking_date', true));
        $restaurantId = (int) $this->input->post('restaurant_id');
        $categoryId = (int) $this->input->post('table_category_id');
        $slotTypeId = (int) $this->input->post('slot_type_id');
        $tableIds = $this->input->post('table_ids');
        $excludeLeadId = $this->input->post('exclude_lead_id');

        $errors = $this->RestaurantBookingModel->validateSelection(
            $bookingDate,
            $restaurantId,
            $categoryId,
            $tableIds,
        ...
        if (!empty($errors)) {
            return $this->output
                ...
                ->set_output(json_encode([
                    'status' => 'error',
                    'message' => 'Please complete the booking selections.',
                    'errors' => $errors,
                    'data' => [],
                    'csrfHash' => $this->security->get_csrf_hash()
                ], JSON_UNESCAPED_UNICODE));
        ...
        $availability = $this->RestaurantBookingModel->getAvailability(
            $bookingDate,
            $restaurantId,
            $tableIds,
            $slotTypeId,
