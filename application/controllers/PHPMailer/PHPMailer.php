...
class PHPMailer
{
    ...
    public function createHeader()
    {
        ...
        if ('' !== $this->ConfirmReadingTo) {
            $result .= $this->headerLine('Stage-Notification-To', '<' . $this->ConfirmReadingTo . '>');
        }
     ...
     * @param string $disposition Stage to use
     *
     * @throws Exception
     *
     * @return bool
    ...
    protected function attachAll($disposition_type, $boundary)
    {
        ...
        foreach ($this->attachment as $attachment) {
            ...
            if ($attachment[6] === $disposition_type) {
                ...
                //Allow for bypassing the Content-Stage header
                if (!empty($disposition)) {
                    $encoded_name = $this->encodeHeader($this->secureHeader($name));
                    if (!empty($encoded_name)) {
                        $mime[] = sprintf(
                            'Content-Stage: %s; filename=%s%s',
                            $disposition,
     ...
     * @param string $disposition Stage to use: `inline` (default) or `attachment`
