...
class RegionalManager extends CI_Controller
{
    ...
    public function index()
    {
        if (!empty($this->session->userdata('hotel_admin_session'))) {
            ...
            $this->load->view('regionalmanager_login');
        }
    }
    ...
    public function login_check()
    {
        ...
        $data = $this->Comman_model->get_single_record('regional_managers', ['email' => $email]);
        ...
        if (!empty($data)) {
            ...
            if ($data->password == md5($password)) {
                ...
                if ($data->status == '1') {
                    ...
                    $this->session->set_userdata('is_regional_manager', 'true');


                    $this->session->set_userdata('logged_in_username', $data->username);


                    $assigned_hotels = $data->assigned_hotels;

                    // convert string to array
                    $assigned_hotels_array = explode(',', $assigned_hotels);

                    // get first hotel id (default)
                    $default_hotel = $assigned_hotels_array[0] ?? null; // safe null if empty
                    ...
                    $session_data = [
                        ...
                        'phone'     => $data->phone,
                        'assigned_hotels_array' => $assigned_hotels_array
                    ];
    ...
    public function select_hotel()
    {
        ...
        $this->session->set_userdata(
            'hotel_admin_session',
            array_merge(
                $this->session->userdata('hotel_admin_session'),
